...
for i = 1:m
    ...
    Delta2 = Delta2 + deltaL' * a2;
    
    % Cost Function
    J = J + (log(a3) * yik  + log(1 - a3) * (1 - yik));
end
...
J = -1/m .* J;
J = J + (lambda / (2 * m)) * (sum(sum(Theta1(:,2:end) .^ 2)) + sum(sum(Theta2(:,2:end) .^ 2)));  % regulatized term
